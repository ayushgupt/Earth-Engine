...
function arealit(i){
  var thold = blank.where(i.gt(3),i);
  var alit = thold.mask(thold);
  //alit = alit.multiply(ee.Image.pixelArea());
  //alit = ee.Image.pixelArea().mask(alit);
  // alit = alit.set('area',i.multiply(ee.Image.pixelArea()));
  alit=i.multiply(ee.Image.pixelArea());
...
Map.addLayer(results.select('stable_lights'),{palette:'red'},'Area Lit, DN>31');
